...
- [x] 7.3 Open the board by hand and confirm the inbox holds the read mail as well as the unread. State it as a comparison rather than a number: the folder is being worked down while this change is in flight, so an absolute row count written here is wrong by the time it is checked — it read 890 messages and 386 rows when this was planned and 444 and 120 a few hours later. Passing is that a message read in the mail client and not archived is on the board, and that the row count is above what the unread messages alone would give (`mail.read` filtered on the read flag answers that at any moment). This is not a suite and is not part of any tier. **Result: confirmed at the board. Read mail that had not been filed was on it. The comparison holds by a wide margin: the board drew the folders' whole contents as 25 rows, while the unread messages alone would have given 15 -- so 10 rows, 40 per cent of the queue, are mail a person had opened and never decided about, which the old rule hid.**
- [x] 7.4 Tick one message on the board, confirm it leaves the view, and confirm in the mail client that it is in the archive and marked read. Then undo, and confirm the row returns and the message is back in the watched folder with the read state it had before. Passing is all four observed. **Result: confirmed at the board -- ticking filed the message and marked it read, and undo brought the row back with the message in the watched folder and its read state as it had been.**
- [x] 7.5 Archive a message by hand in the mail client, reload the board, and confirm the row goes once the mirror catches up — the other half of what "leaves the queue by leaving the folder" promises. **Result: confirmed at the board -- a message archived by hand in the mail client left the board once the mirror had caught up.**
