...
- [ ] 1.1 Capture the baseline before touching anything: run the self-contained
      ...
      first edit and never re-taken afterwards.
- [ ] 1.2 Add `pytest` to `requirements.txt` and install it into `.venv`.
...
- [ ] 2.1 Add `run_parts(parts, ok)` to `tests/harness.py`: call each part
      ...
      same exit code as the baseline line for it.
- [ ] 2.2 Add `tests/conftest.py`: collect a suite file by importing it and
      ...
      tests/stub/t_window.py`: 16 items reported by part name, all passing.
- [ ] 2.3 Print the `N/M checks passed` line from a session hook in the
      ...
      line — identical.
- [ ] 2.4 Add `pytest.ini` with `testpaths = tests/stub` and
...
- [ ] 3.1 Batch A (t_add, t_bands, t_block, t_bulk, t_busy, t_cal, t_copy,
      t_crash, t_elapsed, t_group2) — counts match the baseline for all ten.
- [ ] 3.2 Batch B (t_green2, t_green3, t_keys2, t_label2, t_mailperf,
      t_mailview, t_mark, t_markup, t_move, t_neigh) — counts match for all ten.
- [ ] 3.3 Batch C (t_note, t_pane, t_paste, t_pastein, t_promote, t_review,
      t_search, t_still, t_top, t_trace) — counts match for all ten.
- [ ] 3.4 Batch D (t_track2, t_track3, t_twins2, t_twins4, t_undo1, t_undo3,
      t_version, t_window, t_work3) — counts match for all nine.
- [ ] 3.5 Batch E (t_work5, t_workspace, t_writes, t_fit) — counts match for all
      four.
...
- [ ] 4.1 t_fit2, t_star, t_star2 (4 module-level checks each) — each suite's
      count matches its baseline line.
- [ ] 4.2 t_green (14), t_twins1 (15), t_title (22) — each count matches.
- [ ] 4.3 t_mailflat (25), t_mail (65) — each count matches.
- [ ] 4.4 t_gateway (141) — count matches. Note this is the stubbed suite of
      that name in `tests/stub/`, not the live `tests/gateway/` tier, which
      this change does not touch.

## 5. The runner

- [ ] 5.1 Make `tests/run.py` launch each self-contained suite as
      ...
      line.
- [ ] 5.2 Make the runner check for pytest before starting a self-contained
      ...
      non-zero.
- [ ] 5.3 Update the project context in `openspec/config.yaml` to say that the
...
- [ ] 6.1 Full tier: `python tests/run.py` reports `52 passed, 0 failed`, and
      every one of the 52 `N/M checks passed` lines is identical to the 1.1
      baseline. Passing is an empty diff between the two lists of 52 lines.
- [ ] 6.2 A part can be named from outside: `.venv/bin/python -m pytest -s
...
- [ ] 6.3 A failing part is re-runnable alone: break one check in one part of
      ...
      `shasum` that it is byte-identical to what it was before the edit.
- [ ] 6.4 Both ways of running a suite agree: for one suite from each shape
      ...
      three.
- [ ] 6.5 Loading a suite runs nothing: import each of the 52 modules in a
      ...
      `FAIL` or `[PASS]` line printed by any of them.
- [ ] 6.6 A dropped part is visible: remove one part from `t_promote`'s `PARTS`
      ...
      baseline and the file's `shasum` matches what it was.
- [ ] 6.7 Soft checks survive: make the first check of one part of `t_writes`
...
- [ ] 6.8 The known-failure machinery still reads a run: with that same
      ...
      `crash:unknown`. Passing is an integer.
- [ ] 6.9 No product code changed: `git diff --name-only` names nothing outside
...
- [ ] 7.1 The configuration tier is untouched: `python tests/run.py --config`
      ...
      involved; this reads the board's own `.env`.
- [ ] 7.2 The store tier is untouched: run two store suites by name through the
...
- [ ] 8.1 On a machine where the repository has been cloned fresh, run
      ...
      still being the run a person can actually perform.
