...
# The suites' own directory, as every other suite here reaches it.
sys.path.insert(0, _TESTS)
from parts import run_parts
import main as M
ok = []
#: The second list: what the source says, as against what the board does.
#: Beside the first because the suite's total has always been the sum.
ok2 = []
def chk(l, c, e=""):
...
#: This suite keeps two lists: one for what the board does, one for what
#: the source says about it.  Its total has always been the sum, so both
#: are named here rather than one of them going uncounted.
CHECKS = (ok, ok2)

if __name__ == "__main__":
    raise SystemExit(run_parts(PARTS, *CHECKS))
